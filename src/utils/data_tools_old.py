...
def load_datasets(dataset, data_path):
    ...
    if dataset.startswith("mnist"):
        ...
        raise NotImplementedError(
            "You requested to load dataset {0!s}, please choose MNIST*, TFD, nottingham, muse, pianomidi, jsb.".format(
                dataset))
...
def dataset1_indices(labels, classes=10):
    ...
    for n in pool:
        if len(n) == 0:
            ...
            print
            print
            "stopped early from dataset1 sequencing - missing some class of labels"
            print
    while not stop:
...
def dataset2_indices(labels, rng, classes=10, change_prob=.5):
    ...
    for n in pool:
        if len(n) == 0:
            stop = True
            print
            "stopped early from dataset2 sequencing - missing some class of labels"
    while not stop:
        for i in range(classes):
            if not stop:
                n = i
...
def dataset2a_indices(labels, classes=10):
    ...
    for n in pool:
        if len(n) == 0:
            ...
            print
            print
            "stopped early from dataset2a sequencing - missing some class of labels"
            print
    while not stop:
...
def dataset2b_indices(labels, classes=2):
    ...
    for n in pool:
        if len(n) == 0:
            ...
            print
            print
            "stopped early from dataset2b sequencing - missing some class of labels"
            print
    while not stop:
        for i in range(classes) + range(classes - 1, -1, -1):
            if not stop:
...
def dataset3_indices(labels, classes=10):
    ...
    for n in pool:
        if len(n) == 0:
            stop = True
            print
            "stopped early from dataset3 sequencing - missing some class of labels"
    a = False
    while not stop:
        for i in range(classes):
            if not stop:
...
def dataset4_indices(labels, classes=10):
    ...
    for n in pool:
        if len(n) == 0:
            stop = True
            print
            "stopped early from dataset4 sequencing - missing some class of labels"
    s = [0, 1, 2]
...
def sequence_mnist_data(train_X, train_Y, valid_X, valid_Y, test_X, test_Y, dataset=1, rng=None, one_hot=False):
    ...
    if one_hot:
        ...
        train = numpy.array([[1 if i == y else 0 for i in range(10)] for y in train_Y.get_value(borrow=True)],
                            dtype="float32")
        train_X.set_value(train)
        valid = numpy.array([[1 if i == y else 0 for i in range(10)] for y in valid_Y.get_value(borrow=True)],
                            dtype="float32")
        valid_X.set_value(valid)
        test = numpy.array([[1 if i == y else 0 for i in range(10)] for y in test_Y.get_value(borrow=True)],
                           dtype="float32")
