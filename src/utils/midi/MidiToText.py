...
class MidiToText(MidiOutStream):
    ...
    def channel_message(self, message_type, channel, data):
        ...
        print
        'message_type:%X, channel:%X, data size:%X' % (message_type, channel, len(data))

    def note_on(self, channel=0, note=0x40, velocity=0x40):
        print
        'note_on  - ch:%02X,  note:%02X,  vel:%02X time:%s' % (channel, note, velocity, self.rel_time())

    def note_off(self, channel=0, note=0x40, velocity=0x40):
        print
        'note_off - ch:%02X,  note:%02X,  vel:%02X time:%s' % (channel, note, velocity, self.rel_time())
    ...
    def continuous_controller(self, channel, controller, value):
        print
        'controller - ch: %02X, cont: #%02X, value: %02X' % (channel, controller, value)

    def patch_change(self, channel, patch):
        print
        'patch_change - ch:%02X, patch:%02X' % (channel, patch)
    ...
    def pitch_bend(self, channel, value):
        print
        'pitch_bend ch:%s, value:%s' % (channel, value)
    ...
    def system_exclusive(self, data):
        print
        'system_exclusive - data size: %s' % len(date)

    def song_position_pointer(self, value):
        print
        'song_position_pointer: %s' % value

    def song_select(self, songNumber):
        print
        'song_select: %s' % songNumber
    ...
    def midi_time_code(self, msg_type, values):
        print
        'midi_time_code - msg_type: %s, values: %s' % (msg_type, values)
    ...
    def header(self, format=0, nTracks=1, division=96):
        print
        'format: %s, nTracks: %s, division: %s' % (format, nTracks, division)
        print
        '----------------------------------'
        print
        ''
    ...
    def sysex_event(self, data):
        print
        'sysex_event - datasize: %X' % len(data)
    ...
    def meta_event(self, meta_type, data):
        print
        'undefined_meta_event:', meta_type, len(data)
