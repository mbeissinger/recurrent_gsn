...
def main():
    ...
    parser.add_argument('--cost_funct', type=str, default='pseudo_log') # the cost function for training
...
def create_rnngsn(args):
    if args.dataset == "nottingham":
        train, valid, test = data.load_nottingham(args.data_path)
    elif args.dataset == "muse":
        train, valid, test = data.load_muse(args.data_path)
    elif args.dataset == "jsb":
        train, valid, test = data.load_jsb(args.data_path)
    else:
        train, valid, test = data.load_piano_midi_de(args.data_path)
