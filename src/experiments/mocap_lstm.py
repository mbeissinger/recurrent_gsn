...
if __name__ == '__main__':
    ...
    model = LSTM(
        input_size=49, hidden_size=128,
        num_layers=2, bias=True, batch_first=False,
        dropout=0, bidirectional=False, output_size=49, output_activation=lambda x: x
    )
    if use_cuda:
        model.cuda()
    print('Model:', model)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, sequence_batch in enumerate(train_loader):
            ...
            for seq in subsequences:
                ...
                acc = []
                p = predictions[:-1].view(batch_size, seq_len-1, 49).contiguous()
        ...
        for batch_idx, sequence_batch in enumerate(test_loader):
            ...
            acc = []
            p = predictions[:-1].view(batch_size, sequence_len - 1, 49).contiguous()
            t = targets.view(batch_size, sequence_len - 1, 49).contiguous()
            for i, px in enumerate(p):
                tx = t[i]
                acc.append(torch.sum((tx - px) ** 2) / len(px))
