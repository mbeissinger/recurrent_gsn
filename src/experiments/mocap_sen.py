...
if __name__ == '__main__':
    ...
    model = SEN(
        in_size=49, hidden_size=128, rnn_hidden_size=128, tied_weights=True, walkbacks=4, visible_act=lambda x: x,
        hidden_act=nn.Tanh(),
    ...
    # gsn_optimizer = optim.Adam(list(model.gsn_0.parameters()) + list(model.gsn_2.parameters()), lr=1e-3)
    # regression_optimizer = optim.Adam(model.parameters(), lr=1e-3)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, sequence_batch in enumerate(train_loader):
            ...
            for seq in subsequences:
                ...
                loss = encode_cost + sum([sum([F.mse_loss(input=recon, target=targets[step]) for recon in recons]) for step, recons in enumerate(target_recons[:-1])])
                loss.backward()
        ...
        with open('_mocap_sen_reg_train.csv', 'a') as f:
            ...
            for line in lines:
                f.write(line)
        with open('_mocap_sen_.csv', 'a') as f:
