...
if __name__ == '__main__':
    ...
    model = RNNGSN(
        sizes=[49, 128, 128], rnn_hidden_size=256, tied_weights=True, walkbacks=4, visible_act=lambda x: x,
        hidden_act=nn.Tanh(),
    ...
    for epoch in range(epochs):
        ...
        with open('_mocap_rnngsn_gsn_train.csv', 'a') as f:
            lines = ['{!s},{!s}\n'.format(loss, acc) for
                     loss, acc in zip(gsn_train_losses, gsn_train_accuracies)]
            for line in lines:
                f.write(line)
        with open('_mocap_rnngsn_reg_train.csv', 'a') as f:
            ...
            for line in lines:
                f.write(line)
        with open('_mocap_rnngsn_.csv', 'a') as f:
