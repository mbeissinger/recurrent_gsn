...
class RNNGSN(nn.Module):
    ...
    def init_rnn_hiddens(self, x):
        ...
        return [
            Variable(
                torch.zeros(batch_size, self.lstm_cell.hidden_size)).cuda() if x.is_cuda  # puts the tensor on gpu if our input is on gpu
            else Variable(torch.zeros(batch_size, self.lstm_cell.hidden_size))
            for _ in range(2)
        ]
