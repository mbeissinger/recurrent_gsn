...
class GSN:
    ...
    def train(self, train_X=None, valid_X=None, test_X=None):
        if train_X is None:
        ...
        f_learn = theano.function(inputs  = [self.X],
                                  updates = updates,
                                  outputs = show_gsn_cost)
    
        f_cost  = theano.function(inputs  = [self.X],
                                  outputs = show_gsn_cost)
        ...
        if self.f_noise is None:
            self.f_noise = theano.function(inputs = [self.X], outputs = salt_and_pepper(self.X, self.input_salt_and_pepper))
    ...
    def test(self, test_X=None):
        ...
        # Create the test cost function if it doesn't exist (haven't already trained)
        if self.f_recon is None:
            log.maybeLog(self.logger, "\nBuilding GSN graph")
            # GSN - no noise to work over test input that is noisy (or real-world)
            add_noise = False
            p_X_chain, _ = build_gsn(self.X,
                                     self.weights_list,
                                     self.bias_list,
            ...
            t = time.time()
            self.f_recon = theano.function(inputs  = [self.X],
                                      outputs = [show_gsn_cost, p_X_chain[-1]])
            self.f_noise = theano.function(inputs = [self.X], outputs = salt_and_pepper(self.X, self.input_salt_and_pepper))
            log.maybeLog(self.logger, "Compiling done. Took "+make_time_units_string(time.time() - t)+".\n")
