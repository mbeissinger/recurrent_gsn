...
def experiment(state, outdir_base='./'):
    ...
    def create_gsn(x_t, b_tm1, noiseflag=True):
        chain = []
        ...
        for i in range(walkbacks):
            logger.log("Walkback {!s}/{!s} for RNN-GSN".format(i+1,walkbacks))
            update_layers(hiddens_t, chain, noiseflag)
    ...
    def build_gsn_given_bias(xs, u, noiseflag=True):
