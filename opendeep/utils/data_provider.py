...
import h5py, os, cPickle, gzip, socket, shutil, warnings, sys, glob
from itertools import izip
from tempfile import mkdtemp
import warnings, numpy, scipy, scipy.io, scipy.sparse
import RAB_tools
...
class DataProvider_KFold(DataProvider):
    def __init__(self, train, train_label, test, test_label,
        ...
        if special:
            ...
            print 'special K Fold with missing labels'
            ...
            print 'ordinary leave k out without misssing labels'
...
class DataProvider_leaveKout(DataProvider):
    def __init__(self, train, train_label, test, test_label,
        ...
        if special:
            print 'special leave k out with missing labels'
            assert numpy.sum(train_label[:,0] == -1) != 0
            train_idx_unsup = train_label[:,0]==-1
            self.train_unsup = train[train_idx_unsup]
            self.train_unsup_label = train_label[train_idx_unsup]
...
class DataProvider_Online(DataProvider):
    ...
    def _preload_first_batch(self):
        print 'Preloading the trainset and validset'
...
class DataProvider_Scalable(DataProvider):
    ...
    def next_fold(self):
        ...
        self._preload_first_batch()
        print 'Loading dataset for the next fold...Success!'

    def _refresh_trainset(self):
        #print ''
        print 'Trainset refreshed!'
...
def load_GRO_fun_batches_1008():
    print 'loading GRO Fun Batches 1008...'
...
def load_face_tubes_batches():
    print 'loading face tubes batches...'
...
def load_TFD(style='unsupervised', normalize=False):
    ...
    test_x = test.X
    test_y = test.y
    print 'full train size: ', full_train_x.shape
    print 'unlabeled size: ', unlabeled_x.shape
    print 'trainset size: ', train_x.shape
    print 'validset size: ', valid_x.shape
    print 'testset size: ', test_x.shape

    
    if style == 'unsupervised':
        print 'TFD unsupervised loaded. unlabeled as train, valid as valid, test as test.'
        ...
        print 'TFD is loaded such that the train_x is the original unsupervised, valid'\
        ...
        print 'train size', train_x_f.shape
        print 'valid size', valid_x_f.shape
        print 'test size', test_x_f.shape
...
def load_GRO_fun_unlabeled(which='744'):
    if which == '744':
        ...
        base_path = None
    print 'loading data for prediction gro fun 1008...'
...
def load_GRO_fun_labeled(mode='normal', which='744', small=False,
    ...
    if which == '744':
        print 'loading gro fun 744 labeled...'
        path = base_path + 'fun_744_labeled_split_70_15_15/'

    elif which == '1008':
        print 'loading gro fun 1008 labeled...'
    ...
    if use_differential_targets:
        ...
        print 'loading labels without player bias... '
        ...
        if 0:
            print 'using artificial targets...'
    ...
    print 'combining pids with attributes, pid appends to attrs...'
...
def preprocess_GRO_kill_ratio_1008():
    ...
    print 'making train_x and train_y'
...
def load_GRO_winner():
    print "loading GRO winner dataset"
    ...
    if 1:
        print 'splitting into train, valid and test'
        id1 = 200000
        #id1 = 20000
...
def load_GRO_kill_ratio_calibration():
    print 'loading gro kill ratio calibration...'
...
def load_cifar10_raw():
    print 'loading cifar10..., no validation set, use testset as validset'
...
def load_stackexchange(small=False):
    ...
    if small:
        print 'loading stackexchange small'
...
def load_biodesix(task, unsupervised=False, mix=False, pretrain=False):
    assert task==[0] or task == [1] or task == [2] or task == [3] or task == [4] or task == [5] or task == [0,1] or task==[0,1,2,3,4,5] or task==[1,2,3,4,5]
    base_path = get_rab_dataset_base_path() + 'biodesix/'
    
    def load_a_task(i):
        assert i in [0,1,2,3,4,5]
        print 'loading biodesix p%d'%i
        if i == 0:
    ...
    if task == [1]:
        ...
        print 'loading all train data, sup+unsup, for pretraining'
        p0_train_x, p0_train_y = load_a_task(0)
        p1_train_x, p1_train_y, p1_test_x, p1_test_y = load_a_task(1)
...
def load_mnist_scaled(binarize=True):
    print 'load MNIST 14 by 14, binarized'
...
def preprocess_mnist_scaled(shuffle=True):
    print 'load original mnist and scaled each example to 14*14'
...
def load_mnist_multitasking():
    ...
    print 'binary MNIST, not the standard split.'
...
def load_mnist(binary=True, standard_split=False,
    ...
    if standard_split:
        print 'combining train and valid, valid is in fact test'
        train_x = numpy.concatenate((train_x,valid_x), axis=0)
...
def load_google_faces_and_emotion_challenge():
    ...
    sequence_length = 1
    print 'loading smooth face tubes from Raul'
...
def format_dataset_for_nnet_gpu(c_ordered=True):
    ...
    for i in range(n_batches_train):
        ...
        if padding and extra_x is not None and extra_y is not None:
            print 'Padding %d samples for the last batch'%len(extra_y)
            if c_ordered:
                extra_x = extra_x.T
        ...
        print 'saving training batch %d of %d'%(i+1, n_batches_train)
    ...
    for i in range(n_batches_valid):
        ...
        print 'saving valid batch %d of %d'%(i+1, n_batches_valid)
    ...
    for i in range(n_batches_test):
        ...
        print 'saving test batch %d of %d'%(i+1, n_batches_test)
...
def preprocess_gro_fun_1008(directory='lisatmp'):
    ...
    if os.path.exists(train_x_path):
        print 'trainset already existed.'
        train_x_all = numpy.load(base_path + 'train_x_labeled.npy')
        train_y_all = numpy.load(base_path + 'train_y_labeled.npy')
    else:
        print 'making train_x and train_y'
    ...
    if os.path.exists(test_x_path):
        print 'testset already existed.'
        test_x_all = numpy.load(base_path + 'test_x_labeled.npy')
        test_y_all = numpy.load(base_path + 'test_y_labeled.npy')
    else:
        print 'making test_x and test_y'
        test_x_all = numpy.load(base_path + 'testset_attrs.npy').astype('float32')
    ...
    print 'total amount of examples: ', all_x.shape[0]
    import pdb; pdb.set_trace()
    if 1:
...
def load_2D_straightline():
    print 'loading 2D straight line'
...
def load_3D_corkscrew():
    print "loading 3D corkscrew dataset"
...
def load_manifold():
    print 'load manifold dataset and push range into [0,1]'
...
def preprocess_gro_fun_744():
    ...
    print 'total number of different players: ', all_ids.shape[0]
    print 'number of different players in the trainset: ', numpy.unique(train_ids).shape[0]
    print 'number of different players in the testset: ', numpy.unique(test_ids).shape[0]
    ...
    for id in test_uids:
        if id in train_uids:
            count +=1
    print 'number of players in the testset but not in the trainset: ', count
    ...
    print 'trainset: ', train_x_all.shape
...
def load_mighty_quest():
    ...
    (83570, 205)
    """
    print 'loading mighty quest diff prediction dataset'
...
def preprocess_facetube_pairs(n_chunks=1):
    print 'generating training paris from Guillaume facetube prepro pipeline...'
...
def preprocess_face_tube(sequence_length=1, size=(48,48)):
    print 'preprocessing face tube: normalizing. and size: ',size
    ...
    print 'number of training examples:', train_x.shape
    print 'number of validation examples:', valid_x.shape
...
def load_facetube_pairs():
    ...
    print 'loaded %d facetube pairs'%n_examples
    #import ipdb; ipdb.set_trace()
    return train_feature_x, train_feature_y
    
def load_face_tubes():
    print 'loading face tubes...'
    ...
    print 'validset: ', valid_x.shape
    print 'testset is the validset!!!'
...
def load_shifted_images(full=False):
    print 'loading shifted images dataset'
...
def preprocess_berkeley():
    ...
    def matching_data_and_ground_truth(lists_ground_truth, lists_data):
        print 'combining data and labels into a object numpy array'
        Ds = []
    ...
    print 'saving numpy to %s'%save_path
...
def libsvm_from_numpy(train_x, train_y, save_path=None, skip_zero=False):
    ...
    print 'formatting from numpy to libsvm'
...
def load_facetube_TS():
    print 'loading facetubes from Guillaume...Testset is Validset'
    ...
    if 1:
        ...
        save_path = get_rab_dataset_base_path() + 'aligned_faces_frame_pairs_by_clip_by_emotion/%s_frame_pairs_per_clip/'%str(n_pairs)
        RAB_tools.create_dir_if_not_exist(save_path)
        print 'saving...'
        numpy.save(save_path+'train_x.npy', train_x)
        numpy.save(save_path+'valid_x.npy', valid_x)
...
def preprocess_texture():
    print 'preprocess Brodatz texture dataset'
    name = 'texture_6'
    how_many = 10000
    patch_size = (28,28)
    base_path = RAB_tools.get_rab_dataset_base_path() + '/textures/brodatz/'
...
def load_brodatz_whole(which_one):
    ...
    print 'loading brodatz pics full resolution'
    base_path = RAB_tools.get_rab_dataset_base_path() + '/textures/brodatz/'
...
def test_dataprovider_online():
    ...
    if x is not None and y is not None:
        print 'received new x and y to set up shared variable'
    else:
        raise NotImplementedError()
    while counter < n_loops:
        ...
        if x is not None and y is not None:
            print 'advanced a chunk, set up shared variable'
